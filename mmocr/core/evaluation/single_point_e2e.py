...
def eval_seal(results, coco_annos, logger=None):
    ...
    for i, img_id in enumerate(img_ids):
        ...
        gt_trans_lists = [an['transcription'] for an in coco_annos.loadAnns(ann_id)]
        # gt_trans = ''.join(gt_trans)

        if len(res) == 0:
            res_trans_lists = []
        else:
            res_trans_lists = [res[idd] for idd in sort_id]
        ...
        for g in gt_trans_lists:
            ...
            if min_dis == 0:
                n_correct_rec += 1
            #                print(g)
            ned = min_dis/(max(len(g), len(pre))+1e-5)
            sum_dis += ned
...
def eval_text(results, coco_annos, logger=None):
